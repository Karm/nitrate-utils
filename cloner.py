...
def processRuns(testrun, product, new_product_version, new_build, new_tester, new_manager):
    logDots(1)
...
if __name__ == "__main__":
    ...
    for testrun in testplan_testruns:
        try:
            processRuns(testrun, options.product, new_product_version, new_build, new_tester, new_manager)
        except xmlrpclib.ProtocolError, err:
            logerror(err)
            for trial in range(0, 5):
                try:
